...
# =========================================================
# Mihomo 全能部署脚本 (四库预装 + Notify通知 + 防卡死)
# =========================================================
...
clear
echo -e "${BLUE}#################################################${NC}"
echo -e "${BLUE}#   Mihomo 裸核网关 (四库预装稳定版 + Notify)   #${NC}"
echo -e "${BLUE}#################################################${NC}"
...
# --- 使用你实测成功的地址，并加上超时重试机制 ---
echo -e "${GREEN}[2/5] 正在下载 Country.mmdb...${NC}"
curl -L --max-time 60 --retry 2 -# -o "$CONF_DIR/Country.mmdb" "https://gh-proxy.com/https://github.com/MetaCubeX/meta-rules-dat/releases/download/latest/country-lite.mmdb"

echo -e "${GREEN}[3/5] 正在下载 geosite.dat...${NC}"
curl -L --max-time 60 --retry 2 -# -o "$CONF_DIR/geosite.dat" "https://gh-proxy.com/https://github.com/MetaCubeX/meta-rules-dat/releases/download/latest/geosite.dat"

echo -e "${GREEN}[4/5] 正在下载 geoip.dat...${NC}"
curl -L --max-time 60 --retry 2 -# -o "$CONF_DIR/geoip.dat" "https://gh-proxy.com/https://github.com/MetaCubeX/meta-rules-dat/releases/download/latest/geoip-lite.dat"

echo -e "${GREEN}[5/5] 正在下载 GeoLite2-ASN.mmdb...${NC}"
curl -L --max-time 60 --retry 2 -# -o "$CONF_DIR/GeoLite2-ASN.mmdb" "https://gh-proxy.com/https://github.com/xishang0128/geoip/releases/download/latest/GeoLite2-ASN.mmdb"

# =========================================================
# 5. 生成自动更新脚本 (✅ 保留配置注入，供后续自动维护)
# =========================================================
...
INNEREOF
    ...
    send_notify "Mihomo 更新成功" "节点与Geo库已更新，ASN规则已修复。时间: $(date)"
else
